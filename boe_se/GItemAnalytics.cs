...
namespace boe_se
{
	namespace Engine
	{
		public partial class GItem
		{
            /* Code mit Arrays
            #region zum Debuggen
            public static long gleitenderDurchschnitt;
            public static long exponentiellerDurchschnitt;
            public static long FormationEinrichten;
            public static long Formationen;
            public static long ArrayBereitstellen;
            #endregion
			...
			public int kurs {
				get {
                    ...
                    watch.Stop();
                    Console.WriteLine(watch.ElapsedMilliseconds + " ms für das Bereitstellen des Arrays");
			...
			private int getAverage ()
			{
				...
				int a = movingAverage ();
                watch.Stop();
                Console.WriteLine(watch.ElapsedMilliseconds + " ms für den gleitenden Durchschnitt");
                gleitenderDurchschnitt += watch.ElapsedMilliseconds;
                watch = new System.Diagnostics.Stopwatch();
                watch.Start();
				int b = expAverage ();
                watch.Stop();
                Console.WriteLine(watch.ElapsedMilliseconds + " ms für den exponentiellen Durchschnitt");
			...
			private int getFormation ()//Parameter: 1. Verkaufszeit 2. Preis 3. Menge // -2  keine Angabe (z.B. nicht genug Werte), -1  fallender Kurs => Verkaufen, 0 nicht Handeln, 1 steigender Kurs => Kaufen
			{
                ...
                watch.Stop();
                Console.WriteLine(watch.ElapsedMilliseconds + " ms zum Einrichten der Werte für die Formationen");
				...
				bool d = getUKS (werte);
                watch.Stop();
                Console.WriteLine(watch.ElapsedMilliseconds + " ms für die Formationen");
                Formationen += watch.ElapsedMilliseconds;
				if (!b && !d) {
					if (a || c) {
						return -1;
					} else {
			...
			{//Graph vom kurzfristigen gleitenden Durchschnitt
			...
			{//Graph vom langfristigen gleitenden Durchschnitt
				var verkaufspreise = compiledItemValues;
                int longma = 200;
			...
			public int[] expAverageGraph ()//Graph vom expotentiellen gleitenden Durchschnitt
			{
				var verkaufspreise = compiledItemValues;
            ...
            */
            ...
            private List<Tuple<int, int>> compiledItemValues;
            public int kurs
            {
                get
                {
            ...
            private List<Tuple<int, int>> GetCompiledItemValues(bool sell = true)
            {
                var ret = new List<Tuple<int, int>>();
            ...
            private int getFormation()//Parameter: 1. Verkaufszeit 2. Preis 3. Menge // -2  keine Angabe (z.B. nicht genug Werte), -1  fallender Kurs => Verkaufen, 0 nicht Handeln, 1 steigender Kurs => Kaufen
            {
                System.Diagnostics.Stopwatch watch = new System.Diagnostics.Stopwatch();
                watch.Start();
                Tuple<bool, List<Tuple<int, int>>, Tuple<int, int>> werte = mm();
                int length = 0;
                foreach (Tuple<int, int> t in werte.Item2)
                {
                    if (t != null)
            ...
            private Tuple<bool, List<Tuple<int, int>>, Tuple<int, int>> mm() //bool = true => vorletzter Punkt ist ein Hochpunkt
            {
                bool b = false;
                var verkaufswerte = compiledItemValues;
                List<Tuple<int, int>> verkaufswerteUnveraendert = new List<Tuple<int,int>>(verkaufswerte);
                Tuple<int, int> vorletzter = verkaufswerteUnveraendert[verkaufswerteUnveraendert.Count - 2];
                for (int i = 1; i <= verkaufswerteUnveraendert.Count - 2; i++)
                {
                    if (i - 1 >= 0)
                    {
                        if ((verkaufswerteUnveraendert[i].Item1 > verkaufswerteUnveraendert[(i - 1)].Item1 && verkaufswerteUnveraendert[i].Item1 > verkaufswerteUnveraendert[(i + 1)].Item1))
                ...
                return new Tuple<bool, List<Tuple<int, int>>, Tuple<int, int>>(b, verkaufswerte, vorletzter);
            }

            private bool getM(Tuple<bool, List<Tuple<int, int>>, Tuple<int, int>> verkaufswerte) //false kein M oder zu wenig Hoch/Tiefpunkte; true ist ein M => verkaufen
            {
                if (!verkaufswerte.Item1)
                {
                    return false;
                }
                List<Tuple<int, int>> ac = verkaufswerte.Item2;
                Tuple<int, int, int>[] werte = new Tuple<int, int, int>[3];
                int j = 2;
                int temp = 0;

                for (int i = verkaufswerte.Item2.Count - 2; i > 0; i--)
                {
                    if (ac[i] != null)
                    {
                        werte[j] = new Tuple<int, int, int>(ac[i].Item1, ac[i].Item2, temp + 1);
                ...
                if ((werte[1].Item1 < verkaufswerte.Item3.Item1) != (werte[1].Item1 < verkaufswerte.Item2[verkaufswerte.Item2.Count - 1].Item1))
                {
                    return true;
                }
                return false;
            ...
            private bool getW(Tuple<bool, List<Tuple<int, int>>, Tuple<int, int>> verkaufswerte) //false kein W oder zu wenig Hoch/Tiefpunkte; true ist ein W => kaufen
            {
                if (verkaufswerte.Item1)
                {
                    return false;
            ...
            private bool getKS(Tuple<bool, List<Tuple<int, int>>, Tuple<int, int>> verkaufswerte) //false keine KS oder zu wenig Hoch/Tiefpunkte; true ist eine Ks => verkaufen
            {
                if (!verkaufswerte.Item1)
                {
                    return false;
                ...
                Tuple<int, int> lastLinePoint = new Tuple<int, int>(werte[0].Item3 + werte[1].Item3 + werte[2].Item3 + werte[3].Item3, (d - b) / (c - a) * (werte[0].Item3 + werte[1].Item3 + werte[2].Item3 + werte[3].Item3) - (d - b) * (werte[0].Item3 + werte[1].Item3 + werte[2].Item3 + werte[3].Item3) / (c - a) + b);
                Tuple<int, int> newLinePoint = new Tuple<int, int>(werte[0].Item3 + werte[1].Item3 + werte[2].Item3 + werte[3].Item3 + 1, (d - b) / (c - a) * (werte[0].Item3 + werte[1].Item3 + werte[2].Item3 + werte[3].Item3 + 1) - (d - b) * (werte[0].Item3 + werte[1].Item3 + werte[2].Item3 + werte[3].Item3 + 1) / (c - a) + b);
                if ((lastLinePoint.Item2 > verkaufswerte.Item3.Item1) != (newLinePoint.Item2 > verkaufswerte.Item2[verkaufswerte.Item2.Count - 1].Item1))
                {
                    return true;
                }
                return false;
            ...
            private bool getUKS(Tuple<bool, List<Tuple<int, int>>, Tuple<int, int>> verkaufswerte) //false keine UKS oder zu wenig Hoch/Tiefpunkte; true ist eine UKS => kaufen
            {
                if (verkaufswerte.Item1)
                {
                    return false;
            ...
            {//Graph vom kurzfristigen gleitenden Durchschnitt
                ...
                var verkaufspreise = compiledItemValues;
                int shortma = 50;
                if (verkaufspreise.Count < shortma)
                {
                    return null;
                }
                int[] shortMA = new int[verkaufspreise.Count - shortma];

                for (int j = shortma - 1; j < verkaufspreise.Count; j++)
                {
                    int temp = 0;
                    for (int i = 0; i < shortma; i++)
                    {
            ...
            {//Graph vom langfristigen gleitenden Durchschnitt
                var verkaufspreise = compiledItemValues;
                int longma = 200;
                if (verkaufspreise.Count < longma)
                {
                    return null;
                }
                int[] longMA = new int[verkaufspreise.Count - longma];

                for (int j = longma - 1; j < verkaufspreise.Count; j++)
                {
                    int temp = 0;
                    for (int i = 0; i < longma; i++)
                    {
            ...
            {//Ergebnis des Analyseinstruments gleit. Durchschnitt
                ...
                for (int i = 0; i < shortma; i++)
                {
                    temp = temp + (verkaufspreise[verkaufspreise.Count - i - 2]).Item1;
                }
                lastSMA = temp / shortma;
                temp = 0;
                for (int i = 0; i < shortma; i++)
                {
                    temp = temp + (verkaufspreise[verkaufspreise.Count - i - 1]).Item1;
                }
                nowSMA = temp / shortma;
                temp = 0;
                for (int i = 0; i < longma; i++)
            ...
            {
                var verkaufspreise = compiledItemValues;
                int[] expMA = new int[verkaufspreise.Count];
                expMA[0] = (verkaufspreise[0]).Item1;
                expMA[1] = expMA[0] + (2 / 3) * ((verkaufspreise[1]).Item1 - expMA[0]);
                for (int j = 2; j < verkaufspreise.Count; j++)
                {
                    expMA[j] = expMA[j - 1] + (2 / (j + 1)) * ((verkaufspreise[j]).Item1 - expMA[j - 1]);
                }
                return expMA;
            ...
            private int expAverage()//Ergebnis des Analyseinstruments exp. gleit. Durchschnitt
            {
                ...
                int stocknow = (verkaufspreise[verkaufspreise.Count - 1]).Item1;
                int stocklast = (verkaufspreise[verkaufspreise.Count - 2]).Item1;
                int expnow = expMA[expMA.Length - 1];
                int explast = expMA[expMA.Length - 2];
                if (explast > stocklast && expnow < stocknow)
                {
